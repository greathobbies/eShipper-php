...
abstract class AuthorizationCache
{
   ...
   * @throws \Exception
   */
  public static function push($config = null, $clientId, $accessToken, $refreshToken, $tokenCreateTime, $tokenExpiresIn, $refreshExpiresIn)
  {
    // Return if not enabled
    if (!self::isEnabled($config)) {
      return;
  ...
  /**
   * Returns the Value of the key if found in given config, or from Eshipper Config Manager
  ...
  {
    $config = ($config && is_array($config)) ? $config : EshipperConfigManager::getInstance()->getConfigHashmap();
