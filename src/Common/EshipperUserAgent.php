 ...
 * EshipperUserAgent generates User Agent for curl requests
 *
 * @package Eshipper\Common
 */
class EshipperUserAgent
{
  ...
  public static function getValue($sdkName, $sdkVersion)
  {
    $featureList = array(
      ...
      'bit=' . self::_getPHPBit(),
      'os=' . str_replace(' ', '_', php_uname('s') . ' EshipperUserAgent.php' . php_uname('r')),
      'machine=' . php_uname('m')
    );
    ...
    return sprintf("EshipperSDK/%s %s (%s)", $sdkName, $sdkVersion, implode('; ', $featureList));
  }
